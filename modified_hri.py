...
class HighRiskItem:
    ...
    # AWS_CREDS = { #cloud
    #     "aws_access_key_id": "AKIA3W3L2N45J5CDMBS7",
    #     "aws_secret_access_key": "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",
    #     "region_name": "us-east-1",
    # }
    # AWS_CREDS = dict()
    AWS_CREDS = {
        "aws_access_key_id": "AKIA6D5JUBTNHTHAQDRY",
        "aws_secret_access_key": "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",
        "region_name": "us-east-1",
    }
    ...
    def s3(self):
        ...
        client = boto3.client("s3", **self.AWS_CREDS)
            ...
            for bucket in response_list_buckets.get("Buckets", []):
                ...
                try:
                    ...
                    if (
                        ...
                        unencrypted_buckets.append({"bucket_name": bucket_name})
        except ClientError as e:
            print(e)
            return
        ...
        if public_bucket_names:
            self.data["AWS S3 buckets with public read/write access"] = {
                ...
                "details": [
                    {"bucket_name": bucket_name} for bucket_name in public_bucket_names
                ],
                self.IDENTIFIER: "bucket_name",
            }
        ...
        for bucket in response_list_buckets.get("Buckets", []):
            ...
            try:
                ...
                if bucket_versioning_status != "Enabled":
                    disabled_buckets_versioning.append({"bucket_name": bucket_name})
        ...
        def find_infrequently_used_s3_buckets():
            ...
            if infrequently_used_bucket_names:
                self.data[
                    "Infrequently used AWS S3 buckets"
                ] = infrequently_used_bucket_names
    ...
    def iam(self):
        ...
        """
        client = boto3.client("iam", **self.AWS_CREDS)
        region =  self.AWS_CREDS["region_name"]
        vpc = "-"
        iam_users = []
        ...
        for iam_user in iam_users:
            try:
                ...
                if e.response.get("Error", {}).get("Code", "") == "NoSuchEntity":
                    users_without_mfa.append({"username": iam_user,"vpc":vpc,"region":region})
                else:
                    print(e)
                    continue
        ...
        # ################################################
        # # Active IAM account key detected (Security) #
        # ################################################
        # active_keys = []
        # active_user_key = []

        # def add_active_keys_to_list(list_access_keys_data: dict):
        #     if list_access_keys_data["AccessKeyMetadata"]:
        #         for access_key_details in list_access_keys_data["AccessKeyMetadata"]:
        #             if access_key_details["Status"] == "Active":
        #                 active_keys.append(access_key_details["AccessKeyId"])
        ...
        #         add_active_keys_to_list(list_access_keys_data=page)
        ...
        #             dt_aware = (datetime.date.today() - accesskeydate)
        #             created_days = dt_aware.days
        #             active_user_key.append({"iam_user":iam_user,"active_keys":active_keys,"":created_days})

        # if active_keys:
        #     # Inactive keys for last 6 days
        #     self.data["Inactive IAM account key detected"] = {
        #         "count": len(active_keys),
        #         "details": active_user_key,
        #         self.IDENTIFIER: "key",
        #     }
                    ...
                    try:
                        ...
                        if last_used:
                            ...
                            if last_used_date < dt_aware:
                                unused_roles.append(
                                    {
                                        "role_name": role["RoleName"],"vpc":vpc,"region":region
                                    }
                                )
                        else:
                            unused_roles.append(
        ...
        for user_policy_data in list_of_user_policy:
            if "AdministratorAccess" in user_policy_data.get("attached_policies", []):
                administrator_access_user.append(
                    {"username": user_policy_data.get("username"),"vpc":vpc,"region":region}
                )
    ...
    def config(self):
        ...
        """
        data = {}
        client = boto3.client("config", **self.AWS_CREDS)
        region =  self.AWS_CREDS["region_name"]
        #######################################
    ...
    def ec2(self):
        ...
        """
        client = boto3.client("ec2", **self.AWS_CREDS)
        region =  self.AWS_CREDS["region_name"]
                ...
                for vpc in page["Vpcs"]:
                    ...
                    try:
                        ...
                        if len(response["FlowLogs"]) == 0:
                            vpc_with_flow_log_disabled.append({"vpc_id": vpc["VpcId"],"vpc":vpc_name,"region":region})     
            ...
            for reservation in page["Reservations"]:
                for instance in reservation["Instances"]:
                    ...
                    if "VpcId" not in instance.keys():
                        ec2_classic_vpc.append({"instance_name": instance_name, "instance_id": instance["InstanceId"],"vpc_id":instance["VpcId"],"subnet_id":instance["SubnetId","region":region]})
            ...
            for volume in page["Volumes"]:
                if not volume["Encrypted"]:
                    ...
                    if 'Tags' in volume:
                        ...
                        # print('-')
                        volume_tag = "-"
                    unencrypted_ebs_volumes.append({"volume_name": volume_tag,"volume_id": volume["VolumeId"],"AvailabilityZone":volume['AvailabilityZone'],"Instance_id":instanceid,"region":region})
                    ...
                    for permission in result.get("CreateVolumePermissions", []):
                        if permission["Group"] == "all":
                            snapshot_with_public_access.append(
                                {"snapshot_name":snapshot_name,"snapshot_id": snapshot["SnapshotId"],"volume_id":snapshot["VolumeId"],"region":region}
                            )
                            break
                except ClientError as e:
                    print(e)
        ...
        for page in describe_volumes_response_iterator:
            for volume in page["Volumes"]:
                ...
                if volume_id not in volumes_with_snapshots:
                    volumes_without_snapshots.append({"volume_name": volume_tag,"volume_id": volume_id,"AvailabilityZone":volume['AvailabilityZone'],"instance_id":instanceid,"region":region})
                ...
                for instance in reservation["Instances"]:
                    ...
                    if not instance["InstanceId"] in instances_with_autoscaling:
                        instances_without_autoscaling.append(
                            {"instance_name": instance_name, "instance_id": instance["InstanceId"],"vpc_id":instance["VpcId"],"subnet_id":instance["SubnetId"],"region":region}
                        )
                            ...
                            if "Events" in instance_status:
                                for event in instance_status["Events"]:
                                    schedule_events_details.append(
                                        {"instance_name": instance_name, "instance_id": instance["InstanceId"],"vpc_id":instance["VpcId"],"subnet_id":instance["SubnetId"],"region":region **event}
                                    )
                    except ClientError as e:
                        print(e)
                ...
                for instance in reservation["Instances"]:
                    if len(instance.get("Tags", [])) == 0:
                        missing_tag_instances.append(
                            {"instance_id": instance["InstanceId"],"vpc_id":instance["VpcId"],"subnet_id":instance["SubnetId"],"region":region}
                        )
            ...
            for volume in page["Volumes"]:
                if len(volume.get("Tags", [])) == 0:
                    missing_tags_for_ebs_resource.append(
                        {"volume_id": volume["VolumeId"],"AvailabilityZone":volume['AvailabilityZone'],"region":region}
                    )
        ...
        for page in describe_images_response_iterator:
            for image in page.get("Images", []):
                ...
                if dt_aware >= creation_date:
                    out_of_date_amis.append({"image_name": image_name,"image_id": image["ImageId"],"region":region,"vpc":"-"})
        ...
        for page in describe_images_response_iterator:
            for image in page.get("Images", []):
                ...
                # print(image_name)
                if image["Public"]:
                    public_amis.append({"image_name": image_name,"image_id": image["ImageId"],"region":region,"vpc":"-"})
                    break
        ...
        for page in describe_volumes_response_iterator:
            for volume in page["Volumes"]:
                ...
                if volume["State"] == "available":
                    unused_ebs_volumes.append({"volume_name": volume_tag,"volume_id": volume["VolumeId"],"AvailabilityZone":volume['AvailabilityZone'],"region":region,"vpc":"-"})
        ...
        try:
            ...
            for eip in eips["Addresses"]:
                if "AssociationId" not in eip:
                    unused_eips.append({"eips_name":eips_name,"allocation_id": eip["AllocationId"],"region":region})
        except ClientError as e:
            print(e)
            ...
            for ec2_instance in ec2_instances:
                ...
                if route_table_id is not None:
                    ...
                    if route_table_id in route_tables_with_igw_attached:
                        ...
                        ec2_with_public_subnets_with_open_ports.append({"ec2_instance":ec2_instance,"region":region})
                else:
                    print(
        ...
        for page in describe_vpcs_response_iterator:
            for vpc in page["Vpcs"]:
                ...
                vpc_cidr_blocks.append({"vpc_name" :vpc_name,"vpc_id": vpc_id, "cidr_block": cidr_block})
    ...
    def rds(self):
        ...
        client = boto3.client("rds", **self.AWS_CREDS)
        region =  self.AWS_CREDS["region_name"]
        ##################################
            ...
            for page in describe_db_snapshots_page_iterator:
                for snapshot in page["DBSnapshots"]:
                    ...
                    public_rds_snapshot.append(
                        {"snapshot_identifier": snapshot_identifier,"Vpc_id": vpc_snapshot, "AvailabilityZone": AvailabilityZone,"region":region}
                    )
        except ClientError as e:
            print(e)
            ...
            for instance in page["DBInstances"]:
                ...
                if rds_with_public_subnets and rds_with_open_port:
                    rds_with_public_subnets_with_open_ports.append(
                        {"db_instance_identifier": db_instance_identifier,"vpc_id":vpc_id,"region":region}
                    )
            ...
            for instance in page["DBInstances"]:
                ...
                if not instance["StorageEncrypted"]:
                    unencrypted_rds_instances.append(
                        {"db_instance_identifier": instance["DBInstanceIdentifier"],"vpc_id":vpc_id,"region":region}
                    )
    ...
    def cloudtrail(self):
        ...
        client = boto3.client("cloudtrail", **self.AWS_CREDS)
    ...
    def workspaces(self):
        ...
        client = boto3.client("workspaces", **self.AWS_CREDS)
    ...
    def elasticache(self):
        ...
        client = boto3.client("elasticache", **self.AWS_CREDS)
    ...
    def dynamodb(self):
        ...
        client = boto3.client("dynamodb", **self.AWS_CREDS)
                ...
                try:
                    ...
                    if "ContinuousBackupsDescription" in backup_response:
                        if (
                            ...
                            disabled_dynamoDB_backup.append({"table_name": table_name})
    ...
    def elbv2(self):
        ...
        client = boto3.client("elbv2", **self.AWS_CREDS)
    ...
    def cloudwatch(self):
        ...
        client = boto3.client("cloudwatch", **self.AWS_CREDS)
        ...
        for instance_id in ec2_instance_ids:
            ...
            if average_cpu_utilisation and average_network_io:
                low_traffic_ec2_instance.append({"instance_id": instance_id})
        ...
        if db_instance_identifiers:
            for db_instance_identifier in db_instance_identifiers:
                ...
                if average_cpu_utilisation:
                    rds_instance_idle.append({"db_instance": db_instance_identifier})
        ...
        dynamodb = boto3.client("dynamodb", **self.AWS_CREDS)
        ...
        for table in table_names:
            ...
            if total_consumed_read_units <= (
                ...
                underutilized_tables.append({"table_name": table_name})
        ...
        if nat_gateways:
            for nat in nat_gateways:
                ...
                if nat_gate_way_state == "available":
                    ...
                    if not bytes_out:
                        unused_nat.append({"nat_id": nat_gate_way_id})
        ...
        redshift = boto3.client("redshift", **self.AWS_CREDS)
        ...
        if ec2_volume_ids:
            for volume_id in ec2_volume_ids:
                ...
                if utilization < 0.7:
                    underutilized_provisioned_iops.append({"volume_id": volume_id})
        ...
        for instance_id in ec2_instance_ids:
            ...
            if (
                ...
                ec2_instance_rightsizing.append({"instance_id": instance_id})
        ...
        for bucket in s3_buckets["Buckets"]:
            ...
            if total_max_object_count < 1 and total_max_size < 0.1:
                s3_recommendation_rightsizing.append({"bucket_name": bucket_name})
        ...
        cloudwatch_logs = boto3.client("logs", **self.AWS_CREDS)
        ...
        efs = boto3.client("efs", **self.AWS_CREDS)
        ...
        ecs = boto3.client("ecs", **self.AWS_CREDS)
        ...
        client_ec2 = boto3.client("ec2", **self.AWS_CREDS)
        ...
        for page in list_ec2_page_iterator:
            for reserve_ec2 in page["Reservations"]:
                ...
                if cpu_utilization_ec2 < 30 and memory_utilization_ec2 < 30:
    ...
    def support(self):
        ...
        client = boto3.client("support", **self.AWS_CREDS)
        region =  self.AWS_CREDS["region_name"]
        vpc = "-" 
        Tags = "-"
        ################################################
        ...
        if disabled_aws_enterprise_support:
            self.data["Disabled AWS Enterprise support"] = {
                "count": 1,
                "details": [{"status": True,"region":region,"vpc":vpc,"Tag":Tags }],
                self.IDENTIFIER: "status",
                self.TYPE: "boolean",
                self.HRI_IF_STATUS_IS: True,
            }
    ...
    def aws_lambda(self):
        ...
        client = boto3.client("lambda", **self.AWS_CREDS)
        region =  self.AWS_CREDS["region_name"]
        vpc = "-"
        ##########################################################################
                ...
                if "KMSKeyArn" in kms_key_arn:
                    if kms_key_arn["KMSKeyArn"] is None:
                        lambda_variables_without_cmk_encryption_enabled.append(
                            {"lambda_function_name": function_name,"lambda_function_region":region,"Vpc_id":vpc}
                        )
            except ClientError as e:
                print(e)
            ...
            try:
                ...
                for statement in policy_dict["Statement"]:
                    if "Principal" in statement and statement["Principal"] == "*":
                        public_lambda.append({"lambda_function_name": function_name,"function_region":region})
...
hri = HighRiskItem()
hri.get_hri_final_data()
pprint.pprint(hri.data)
