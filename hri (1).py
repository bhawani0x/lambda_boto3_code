...
class HighRiskItem:
        ...
        for page in list_functions_page_iterator:
            if page:
                for function in page["Functions"]:
                    ...
                    try:
                        lambda_response_tags = client.list_tags(
                            Resource=function_arn
                        )["Tags"]
                        if function_name not in lambda_tags:
                            lambda_tags[function_name] = lambda_response_tags
                        if "vpc" in function:
                            if function_name not in lambda_vpcs:
                                lambda_vpcs[function_name] = function["vpc"]
                    except ClientError as e:
                        continue
                        ...
                        lambda_variables_without_cmk_encryption_enabled.append(
                            {
                                ...
                                "region_name": region,
                                "tags": lambda_tags[function_name],
                                "vpc": lambda_vpcs[function_name]
                            }
                        )
            except ClientError as e:
                print(e)
    ...
    def get_hri_final_data(self, region):
        ...
        # original line
        # return new_data
        return all_hris_data

    def get_all_hris_data(self):
        error_message = None
        try:
            ec2 = boto3.client("ec2", **self.AWS_CREDS)
            ...
            print("Could not connect to AWS endpoint URL")
            error_message = str(e)
            region_names = []

        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(self.get_hri_final_data, region)
                for region in region_names
            ]
            # Collect the data returned by each task in a list
            all_hris_data = [
                future.result() for future in concurrent.futures.as_completed(futures)
            ]
            concurrent.futures.wait(futures)

        return all_hris_data, error_message


hri = HighRiskItem()
# Call the method and print the output
all_hris_data, error_message = hri.get_all_hris_data()

if error_message is not None:
    print(f"Encountered error: {error_message}")
else:
    pprint.pprint(all_hris_data)
