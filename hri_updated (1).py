...
class HighRiskItem:
    ...
    def iam(self, region: str):
        ...
        for page in list_users_response_iterator:
            for user in page["Users"]:
                ...
                try:
                    ...
                    tags = []
                if user_arn not in iam_tags_cache:
                    try:
                        iam_tags_cache[user_arn] = tags
                    except ClientError as e:
                        print(e)
        ...
        for iam_user in iam_users:
            try:
                ...
                response_tags = client.list_user_tags(UserName=iam_user)
                try:
                    tags = response_tags["Tags"]
                except:
                    tags = []
                if iam_user not in iam_tags_cache:
                    iam_tags_cache[iam_user] = tags
        ...
        if inactive_keys:
            ...
            for inactive_key in inactive_keys:
                ...
                try:
                    response_tags = client.list_user_tags(UserName=username)
                    try:
                        tags = response_tags["Tags"]
                    except:
                        tags = []
                    ...
                    tags = []
                if inactive_key not in iam_tags_cache:
                    try:
                        iam_tags_cache[inactive_key] = tags
                    except ClientError as e:
                        print(e)
        ...
        for iam_user in iam_users:
            ...
            for attached_policy_names in attached_policy_names:
                if attached_policy_names not in iam_tags_cache:
                    try:
                        iam_tags_cache[iam_user] = tags
                    except ClientError as e:
                        print(e)
            ...
            if page:
                for role in page["Roles"]:
                    try:
                        ...
                        tags = []
                    role_arn = role["Arn"]
                    if role_arn not in iam_tags_cache:
                        try:
                            iam_tags_cache[role_arn] = tags
                        except ClientError as e:
                            print(e)
                    try:
                ...
                administrator_access_user.append(
                    {
                        ...
                        "filter_region": region,
                        "filter_tags": user_policy_data.get("filter_tags"),
                        "filter_vpc": vpc,
                    }
                )
        ...
        try:
            ...
            for status in status:
                try:
                    ...
                    tags = []
                if tags not in config_tags_cache:
                    config_tags_cache[f"config_name-{region}"] = tags
    ...
    def ec2(self, region: str):
            ...
            if page:
                for vpc in page["Vpcs"]:
                    ...
                    try:
                        ...
                        vpc_id = "-"       
                    if vpc_id not in ec2_tags_cache:
                        ec2_tags_cache[vpc_id]= vpc_tags
                    if vpc_id not in ec2_vpc_cache:
                        ec2_vpc_cache[vpc_id]= vpc_id
                    try:
            ...
            for reservation in page["Reservations"]:
                for instance in reservation["Instances"]:
                    ...
                    try:
                        ...
                        instance_tags = [] 
                    if instance["InstanceId"] not in ec2_tags_cache:
                        ec2_tags_cache[instance["InstanceId"]] = instance_tags
                    if instance["InstanceId"] not in ec2_vpc_cache:
                        ec2_vpc_cache[instance["InstanceId"]] = instance_vpc
                        ...
                        ec2_classic_vpc.append(
                            {
                                ...
                                "filter_region": region,
                                "filter_tags": instance_tags,
                                "filter_vpc": instance_vpc,
                            }
                        )
        ...
        for page in describe_volumes_response_iterator:
            for volume in page["Volumes"]:
                if not volume["Encrypted"]:
                    try:
                        volume_tag = volume["Tags"]
                    except:
                        volume_tag = [] 
                   
                    if volume["VolumeId"] not in ec2_tags_cache:
                        ec2_tags_cache[volume["VolumeId"]] = volume_tag
        ...
        for page in describe_snapshots_response_iterator:
            for snapshot in page.get("Snapshots", []):
                ...
                try:
                    ...
                    if snapshot["SnapshotId"] not in ec2_tags_cache:
                        ec2_tags_cache[snapshot["SnapshotId"]] = snapshot_tags
        ...
        for page in describe_volumes_response_iterator:
            for volume in page["Volumes"]:
                ...
                try:
                    ...
                    volume_tag = []
                if volume_id not in ec2_tags_cache:
                    ec2_tags_cache[volume_id] = volume_tag
        ...
        for page in describe_images_response_iterator:
            for image in page.get("Images", []):
                ...
                if image["ImageId"] not in ec2_tags_cache:
                    ec2_tags_cache[image["ImageId"]] = image_tags
        ...
        try:
            ...
            for eip in eips["Addresses"]:
                try:
                    ...
                    eip_tags = []
                if eip["AllocationId"] not in ec2_tags_cache:
                    ec2_tags_cache[eip["AllocationId"]] = eip_tags
                if "AssociationId" not in eip:
                    unused_eips.append(
                        {
                            "allocation_id": eip["AllocationId"],
                            "filter_region": region,
                            "filter_tags": eip_tags,
                            "filter_vpc": "-",
                        }
                    )
        except ClientError as e:
        ...
        for page in describe_route_tables_response_iterator:
            for route_table in page.get("RouteTables", []):
                ...
                if route_table_id not in ec2_tags_cache:
                    ec2_tags_cache[route_table_id] = tags
        ...
        for group_id in list(group_ids_to_check):
            ...
            for ec2_instance in ec2_instances:
                ...
                tags = ec2_instance.get("filter_tags")
        ...
        try:
            ...
            for page in describe_db_snapshots_page_iterator:
                for snapshot in page["DBSnapshots"]:
                    ...
                    snapshot_vpc = snapshot["VpcId"]
                    AvailabilityZone = snapshot["AvailabilityZone"]
                    try:
                        snapshot_tags = snapshot["TagList"]
                    except:
                        snapshot_tags = []
                    if snapshot_tags not in rds_tags_cache:
                        rds_tags_cache[snapshot_identifier] = snapshot_tags
                    if snapshot_vpc not in rds_vpc_cache:
                        rds_vpc_cache[snapshot_identifier] = snapshot_vpc
        ...
        for page in describe_db_instances_page_iterator:
            for instance in page["DBInstances"]:
                ...
                try:
                    ...
                    tags = []
                if db_instance_identifier not in rds_tags_cache:
                    rds_tags_cache[db_instance_identifier] = tags
                if db_instance_identifier not in rds_vpc_cache:
                    rds_vpc_cache[db_instance_identifier] = vpc_id
    ...
    def elasticache(self, region: str):
        ...
        for page in describe_replication_groups_page_iterator:
            for replicationgroup in page["ReplicationGroups"]:
                ...
                try:
                    tag_resource = client.list_tags_for_resource(ResourceName=arn)
                    try:
                        elasticache_tags = tag_resource['TagList']
                    except:
                        elasticache_tags = []
                except:
                    elasticache_tags = []
                if elasticache_tags not in elasticache_tags_cache:
                    try:
                        elasticache_tags_cache[arn] = elasticache_tags
                    except ClientError as e:
                            print(e)
        ...
        try:
            ...
            for web_acl in list_web_acls["WebACLs"]:
                ...
                try:
                    ...
                    if not response:
                        waf_without_resource_attached.append(
                            {
    ...
    def cloudwatch(self, region: str):
                ...
                low_traffic_ec2_instance.append(
                    {
                        ...
                        "filter_region": region,
                        "filter_tags": cloudwatch_tags_cache[instance_id],
                        "filter_vpc": cloudwatch_vpc_cache[instance_id],
                    }
                )
                    ...
                    rds_instance_idle.append(
                        {
                            ...
                            "filter_region": region,
                            "filter_tags": cloudwatch_tags_cache[
                                db_instance_identifier
                            ],
                            "filter_vpc": cloudwatch_vpc_cache[db_instance_identifier],
                        }
                    )
                ...
                under_utilized_rds_instance.append(
                    {
                        "db_instance_identifier": db_instance_identifier,
                        "filter_region": region,
                        "filter_tags": cloudwatch_tags_cache[db_instance_identifier],
                        "filter_vpc": cloudwatch_vpc_cache[db_instance_identifier],
                    }
                )
                        ...
                        unused_nat.append(
                            {
                                ...
                                "filter_region": region,
                                "filter_tags": nat_gate_way_tags,
                                "filter_vpc": nat_gate_way_vpc,
                            }
                        )
        ...
        for cluster_id in redshift_cluster_identifiers:
            ...
            if utilization and total_read_iops < 100 and total_write_iops < 100:
                under_utilized_redshift.append(
                    {
                    ...
                    underutilized_provisioned_iops.append(
                        {
                            "volume_id": volume_id,
                            "filter_region": region,
                            "filter_tags": cloudwatch_tags_cache[volume_id],
                            "filter_vpc": "-",
                        }
                    )
                        ...
                        underutilized_cloudWatch_log_group.append(
                            {
                                ...
                                "filter_region": region,
                                "filter_tags": cloudwatch_tags_cache[log_group_name],
                                "filter_vpc": "-",
                            }
                        )
        except ClientError as e:
                    ...
                    infrequently_accessed_efs_resource.append(
                        {
                            ...
                            "filter_region": region,
                            "filter_tags": cloudwatch_tags_cache[file_system_id],
                            "filter_vpc": "-",
                        }
                    )
                    ...
                    underutilized_aws_ecs_cluster.append(
                        {
                            ...
                            "filter_region": region,
                            "filter_tags": cloudwatch_tags_cache[cluster_name],
                            "filter_vpc": "-",
                        }
                    )
                    ...
                    underutilized_ec2_instances.append(
                        {
                            ...
                            "filter_vpc": cloudwatch_vpc_cache[instance["InstanceId"]],
                        }
                    )
    ...
    def support(self, region: str):
        ...
        describe_response = client.describe_cases()
    ...
    def aws_lambda(self, region: str):
        ...
        for function_name in function_names:
            try:
                ...
                try:
                    vpc_id = response["Configuration"]["VpcConfig"]["VpcId"]
                except:
                    vpc_id = "-"
                try:
                    tags = response["Tags"]
                    ...
                    tags = []
                if function_arn not in lambda_vpc_cache:
                    lambda_vpc_cache[function_arn] = vpc_id
                if function_arn not in lambda_tags_cache:
                    lambda_tags_cache[function_arn] = tags
    ...
    def get_data_by_thread(self, region):
        ...
        # data = self.all_data.append(self.data)
        # print(data)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
    for region in ["us-east-1"]:
        executor.submit(hri.get_data_by_thread, region)
